...
class AcadosCustomOcp:
    ...
    def setup_acados_ocp(self):
        ...
        self.zeta_0 = np.copy(common.init_zeta)
        ...
        ocp.parameter_values = np.array([common.T_f_initial])
        ...
        self.zeta_N = ca.repmat(np.reshape(self.zeta_0, (self.nx,1)), 1, common.N+1)
        self.u_N = ca.repmat(common.U_REF.reshape((self.nu,1)), 1, common.N)
        ...
        ocp.cost.yref = np.concatenate([common.X_REF, common.U_REF, np.zeros(3), np.array([0.0])])

        # block-diag weights: Q on the state portion, R on the input portion
        ocp.cost.W = scipy.linalg.block_diag(common.Q, common.R, common.S, np.array([[common.W_Tf]]))
        ...
        for i in range(common.DRONE_COUNT):
            u0 = 4*i
            lbu[u0]   = common.minThrust
            ubu[u0]   = common.maxThrust
            lbu[u0+1:u0+4] = -common.maxTorquePitchRoll
            ubu[u0+1:u0+4] = common.maxTorquePitchRoll
        ...
        #     lbx.append(np.pi/2 - common.maxCableAngle)
        #     ubx.append(np.pi/2 + common.maxCableAngle)
        #     idxbx.append(phi_idx)
        #     lbx.append(np.pi - common.maxCableAngle)
        #     ubx.append(np.pi + common.maxCableAngle)
        ...
        ocp.solver_options.tf                           = common.T_f  # free-final-time parameter
        ...
        # create solver
        self.ocp = ocp
        self.solver = AcadosOcpSolver(ocp, json_file=os.path.join(self.build_dir, "planner_ocp.json"))
        self.integrator = AcadosSimSolver(ocp)

        # print("init_zeta =", common.init_zeta)
        # print("lbx =", ocp.constraints.lbx)
        # print("ubx =", ocp.constraints.ubx)
        # print("Is in bounds:", np.all(common.init_zeta >= ocp.constraints.lbx), np.all(common.init_zeta <= ocp.constraints.ubx))
        # print("T_f_initial =", common.T_f_initial)
    ...
    def cost_update_ref(self, zeta_0, _):
        ...
        if self.i0 >= len(common.x_ref) - 1:
            return True

        # Advance index using your improved window
        self.advance_track_index(x0, y0, z0, common.x_ref, common.y_ref, common.z_ref, common.N_lookahead)

        # print(f"[cost_update_ref] i0={self.i0}, pos_ref={common.x_ref[self.i0]:.3f},{common.y_ref[self.i0]:.3f},{common.z_ref[self.i0]:.3f}")

        for j in range(common.N):

            idx = min(self.i0 + j, len(common.x_ref) - 2)  # -2 to allow next_idx below
            next_idx = idx + 1

            xj, yj, zj = common.x_ref[idx], common.y_ref[idx], common.z_ref[idx]
            xj_next, yj_next, zj_next = common.x_ref[next_idx], common.y_ref[next_idx], common.z_ref[next_idx]
            ...
            if norm > 1e-6:
                ...
                v_dir = np.zeros(3)
            v_ref = common.velocity_ref * v_dir

            # Assemble reference
            full_ref = np.concatenate([common.X_REF, common.U_REF, np.zeros(3), np.array([common.T_f_initial])])
        ...
        # Terminal node
        idx_T = min(self.i0 + common.N, len(common.x_ref)-1)
        xT, yT, zT = common.x_ref[idx_T], common.y_ref[idx_T], common.z_ref[idx_T]
        yref_e = common.X_REF.copy()
        yref_e[0:3] = [xT, yT, zT]
        self.solver.set(common.N, "yref", yref_e)
