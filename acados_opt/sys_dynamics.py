...
class SysDyn:
    def __init__(self):
        self.n_drones = common.DRONE_COUNT
    ...
    def SetupOde(self):
        ...
        payload_force = ca.vertcat(0, 0, -common.m_payload * common.g0)
        ...
        for i in range(self.n_drones):
            ...
            F_d = T*thrust_dir + common.m_drone*ca.vertcat(0,0,-common.g0)
            ...
            θdd = (fθ / (common.l*common.m_drone)) + φd**2 * ca.sin(θ) * ca.cos(θ)
            φdd = ((fφ / (common.l*common.m_drone)) - 2 * φd * θd * ca.cos(θ)) / ca.sin(θ)
            ...
            α_d = common.droneInertiaInv @ (τ - common.droneBeta * ω)

            # Accumulate into payload wrench
            if self.n_drones == 1:
                corner = quat_to_rot(p_quat) @ ca.DM([0, 0, common.payload_height / 2])
            else:
                # Define corner points for 4 drones
                w = common.payload_width
                d = common.payload_depth
                h = common.payload_height
                corners_local = [
                    ca.DM([-w / 2, -d / 2, h / 2]),
                    ca.DM([ w / 2, -d / 2, h / 2]),
                    ca.DM([ w / 2,  d / 2, h / 2]),
        ...
        a_p = (payload_force - common.payloadBeta*p_vel)/common.m_payload
        α_p = common.payloadInertiaInv @ payload_torque
