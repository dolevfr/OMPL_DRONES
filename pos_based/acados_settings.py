...
class AcadosCustomOcp:
    ...
    def advance_track_index(self, current_value, ref_traj, N_lookahead=0, window_size=15):
        """
        Advances the index (self.i0) along a generic trajectory (ref_traj),
        picking the nearest point in a sliding window, plus N_lookahead.
        """
        n = len(ref_traj)
        ...
        window_refs = ref_traj[window_start:window_end]

        # Compute squared distances for all candidates in the window
        dists = np.sum((window_refs - current_value)**2, axis=1)
        ...
        self.last_nn_idx = new_nn_idx


    def cost_update_ref(self, zeta_0, ref_traj):
        """
        ...
        ref_traj: shape (N, 3), with ref_traj[:,0]=x_ref, [:,1]=y_ref, [:,2]=z_ref
        """
        # Unpack current position from state
        pos0 = np.array([float(zeta_0[0]), float(zeta_0[1]), float(zeta_0[2])])

        n_refs = ref_traj.shape[0]
        if self.i0 >= n_refs - 1:
            return True

        # Advance index using improved window (same as before)
        self.advance_track_index(pos0, ref_traj, N_lookahead=N_lookahead, window_size=100)

        idxs = np.arange(self.i0, self.i0 + N)
        idxs = np.clip(idxs, 0, n_refs - 2)  # -2 so next_idx doesn't go out of bounds

        for j, idx in enumerate(idxs):
            next_idx = min(idx + 1, n_refs - 1)

            # Reference positions
            pos_j = ref_traj[idx]
            pos_j_next = ref_traj[next_idx]

            # Velocity direction from idx to next_idx
            vec = pos_j_next - pos_j
            ...
            # Assemble reference vector
            full_ref = np.concatenate([X_REF, U_REF, np.zeros(3), np.array([T_f_initial])])
            full_ref[0:3] = pos_j
            full_ref[7:10] = v_ref

            # print(f"[cost_update_ref] stage {j}: idx={idx}, pos_ref=({pos_j[0]:.3f}, {pos_j[1]:.3f}, {pos_j[2]:.3f})")
        ...
        # Terminal node
        idx_T = min(self.i0 + N, n_refs-1)
        pos_T = ref_traj[idx_T]
        yref_e = X_REF.copy()
        yref_e[0:3] = pos_T
