...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def openEnvironment(self):
        ...
        env = self.omplSetup.renderEnvironment()
        # print("Rendered environment mesh:", env)
        self.mainWidget.glViewer.setEnvironment(env)
        self.resetBounds()
        if self.isGeometric and self.robotFile:
            resolution = self.omplSetup.getSpaceInformation().getStateValidityCheckingResolution()
            self.mainWidget.plannerWidget.geometricPlanning.resolution.setValue(resolution)
            # print("Set state validity checking resolution to:", resolution)
        self.mainWidget.glViewer.updateGL()  # Refresh the display
    ...
    def openRobot(self):
        ...
        self.omplSetup.setRobotMesh(fname_box)
        robot_box = self.omplSetup.renderRobot()
        # print("Rendered robot_box:", robot_box)
        self.mainWidget.glViewer.robot_box = robot_box

        # print("Loading robot mesh: drone from", fname_drone)
        self.omplSetup.setRobotMesh(fname_drone)
        robot_drone = self.omplSetup.renderRobot()
        # print("Rendered robot_drone:", robot_drone)
        self.mainWidget.glViewer.robot_drone = robot_drone

        self.omplSetup.inferEnvironmentBounds()
        
        # Set the default start and goal state based on the first geometric component.
        ...
        if self.isGeometric:
            resolution = self.omplSetup.getSpaceInformation().getStateValidityCheckingResolution()
            self.mainWidget.plannerWidget.geometricPlanning.resolution.setValue(resolution)
            # print("Set geometric planning resolution to:", resolution)
        
        bounds = self.omplSetup.getGeometricComponentStateSpace().getBounds()
        self.mainWidget.glViewer.setBounds(bounds)
        # print("Set bounds to:", bounds)
        ...
        try:
            ...
            with open(fname, 'r') as f:
                for line in f:
                    ...
                    # Check that the number of values is a multiple of 7 (each state is SE(3))
                    if len(values) % 7 != 0:
                        # print("Error: Invalid state format in path file!")
                        raise ValueError("Each line must contain a multiple of 7 values for SE(3) states.")
                    num_robots = len(values) // 7
                    if len(path) == 0:
            ...
            # Update start and goal poses based on the loaded path.
            bounds = self.mainWidget.glViewer.getBounds()
            for robot_idx in range(num_robots):
                self.mainWidget.problemWidget.setStartPose(self.path[robot_idx][0], self.is3D)
                self.mainWidget.problemWidget.setGoalPose(self.path[robot_idx][-1], self.is3D)
            self.mainWidget.glViewer.setBounds(bounds)
            self.mainWidget.glViewer.updateGL()
        except Exception as e:
            print("Error loading fixed path:", e)
    ...
    def openPath(self):
        ...
        if fname:
            ...
            for line in open(fname, 'r').readlines():
                ...
                # Ensure the number of elements is a multiple of 7 (for multiple SE(3) states)
                if len(values) % 7 != 0:
                    OMPL_ERROR("Invalid state format in path file!")
                    raise ValueError("Each line must contain a multiple of 7 values for SE(3) states.")

                num_robots = len(values) // 7  # Compute the number of robots per line

                if len(path) == 0:  # Initialize storage for each robot's path
                    path = [[] for _ in range(num_robots)]
            ...
            # Send the multi-robot paths to GLViewer
            self.mainWidget.glViewer.setSolutionPaths(self.path)

            # Set start and goal poses for each robot
            bounds = self.mainWidget.glViewer.getBounds()
            for robot_idx in range(num_robots):
                self.mainWidget.problemWidget.setStartPose(self.path[robot_idx][0], self.is3D)
                self.mainWidget.problemWidget.setGoalPose(self.path[robot_idx][-1], self.is3D)
    ...
    def setSolutionPath(self, path):
        ...
        num_robots = len(path.getState(0)) // 7  # Assuming each robot has 7 SE(3) values

        # Initialize storage for each robot's path
        self.path = [[] for _ in range(num_robots)]

        for i in range(ns):
            state = path.getState(i)
            for robot_idx in range(num_robots):
                robot_state = ob.State(self.omplSetup.getGeometricComponentStateSpace(), state)
                self.path[robot_idx].append(
                    self.omplSetup.getGeometricComponentState(robot_state, robot_idx)
                )

        # Send multiple paths to GLViewer
        self.mainWidget.glViewer.setSolutionPaths(self.path)


    def randMotion(self):
        """Generate random motion for multiple robots."""
        self.configureApp()

        random_paths = []  # Store generated paths for all robots

        for _ in range(len(self.path)):  # Loop for each robot
            if self.isGeometric:
                pg = og.PathGeometric(self.omplSetup.getSpaceInformation())
                if pg.randomValid(100):
                    pg.interpolate()
                    random_paths.append(pg.getStates())  # Store random path
                else:
                    OMPL_ERROR("Unable to generate random valid path")
                    random_paths.append([])  # Ensure consistent list size
            else:
                pc = oc.PathControl(self.omplSetup.getSpaceInformation())
                if pc.randomValid(100):
                    random_paths.append(pc.asGeometric().getStates())  # Store path
                else:
                    OMPL_ERROR("Unable to generate random valid path")
                    random_paths.append([])  # Ensure consistent list size

        self.mainWidget.glViewer.setSolutionPaths(random_paths)  # Update multiple paths
    ...
    def configureApp(self):
        ...
        # Retrieve all start and goal poses for multiple robots
        startPoses = [self.omplSetup.getFullStateFromGeometricComponent(
            self.mainWidget.problemWidget.getStartPose(i)) for i in range(len(self.path))]

        goalPoses = [self.omplSetup.getFullStateFromGeometricComponent(
            self.mainWidget.problemWidget.getGoalPose(i)) for i in range(len(self.path))]
        ...
        if self.is3D:
            bounds = ob.RealVectorBounds(3)
            bounds.low[:] = self.mainWidget.glViewer.bounds_low
            bounds.high[:] = self.mainWidget.glViewer.bounds_high
        else:
            bounds = ob.RealVectorBounds(2)
            bounds.low[:2] = self.mainWidget.glViewer.bounds_low[:2]
            bounds.high[:2] = self.mainWidget.glViewer.bounds_high[:2]

        self.omplSetup.getGeometricComponentStateSpace().setBounds(bounds)

        # Set start and goal states for all robots
        if self.isGeometric:
...
class GLViewer(QtOpenGL.QGLWidget):
    ...
    def setStartPose(self, values):
        """Set start poses for multiple robots."""
        if not isinstance(values, list):
            values = [values]
        # If values is a flat list of numbers, wrap it into a list-of-one.
        if values and isinstance(values[0], (int, float)):
            values = [values]
        
        self.startPose = values  # Store multiple start poses

        # Update bounds based on all robot positions
        for value in values:
            # print("Updating bounds using start pose:", value)
            self.updateBounds(value[3:])  # Expecting value to be a list with at least 4 elements

        self.updateGL()  # Refresh OpenGL scene

    def setGoalPose(self, values):
        """Set goal poses for multiple robots."""
        if not isinstance(values, list):
            values = [values]
        # If values is a flat list of numbers, wrap it into a list-of-one.
        if values and isinstance(values[0], (int, float)):
        ...
        self.goalPose = values  # Store multiple goal poses

        # Update bounds based on all robot positions
        for value in values:
            # print("Updating bounds using goal pose:", value)
            self.updateBounds(value[3:])  # Expecting value to be a list with at least 4 elements
    ...
    def updatePathPose(self):
        """Update path pose for all robots during animation."""
        if self.solutionPaths:
            max_length = max(len(path) for path in self.solutionPaths)  # Find the longest path
            self.pathIndex = (self.pathIndex + 1) % max_length  # Loop animation index safely
            self.updateGL()  # Refresh OpenGL scene

    def setSolutionPaths(self, paths):
        """Store multiple robot trajectories."""
        self.solutionPaths = []
        for path in paths:
            self.solutionPaths.append([self.getTransform(state()) for state in path])

        self.pathIndex = 0  # Reset animation index
        self.update()
...
if __name__ == '__main__':
    ...
    window = MainWindow()
    window.show()
    # Delay the calls so the GL context is ready:
    QtCore.QTimer.singleShot(0, window.openEnvironment)
    QtCore.QTimer.singleShot(0, window.openRobot)
    QtCore.QTimer.singleShot(0, window.openFixedPath)
    sys.exit(app.exec_())
