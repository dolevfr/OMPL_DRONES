...
class AcadosCustomOcp:
    ...
    def cost_update_ref(self, zeta_0, u, T_f_value, accel_fun, accel_traj):
        """
        ...
        payload_accel = accel_fun(zeta_0, u, T_f_value).full().flatten()
        # print("==========================")
        # print("Current i0:", self.i0)
        # print("Payload accel ref: ", ["{:.6f}".format(val) for val in accel_traj[self.i0]])
        # print("Current payload accel: ", ["{:.6f}".format(val) for val in payload_accel])
        # print("==========================")

        self.advance_track_index(payload_accel, accel_traj, N_lookahead=N_lookahead)

        accel_start = len(X_REF) + len(U_REF)
        accel_end = accel_start + 3

        for j in range(N):

            idx = min(self.i0 + j, len(accel_traj) - 1)  # -1 to avoid out-of-bounds

            # Assemble reference
            full_ref = np.concatenate([X_REF, U_REF, np.zeros(3), np.array([T_f_initial])])

            # Set desired terminal acceleration in reference
            full_ref[accel_start:accel_end] = accel_traj[idx]
