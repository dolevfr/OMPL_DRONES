...
void payloadSystemSetup(app::PayloadSystem &setup)
{
    ...
    // Set the acceleration subspace to zero
    unsigned int accelIndex = stateSpace->as<base::CompoundStateSpace>()->getSubspaceCount() - 1;
    for (unsigned int i = 0; i < 3; ++i)
    {
        start->as<base::RealVectorStateSpace::StateType>(accelIndex)->values[i] = 0.0;
    }
    ...
    for (unsigned int i = 0; i < 3; ++i)
    {
        goal->as<base::RealVectorStateSpace::StateType>(accelIndex)->values[i] = 0.0;
    }   
...
void payloadSystemDemo(app::PayloadSystem &setup)
{
    ...
    // if (setup.solve(ompl::base::IterationTerminationCondition(1)))
    {
...
int main(int argc, char** argv)
{
    ...
    // system("python3 ../src/python/extract_se3.py solution_path.txt ../src/python/trajectory_se3.txt");
