...
namespace ompl
{
    namespace app
    {
        ...
        protected:
            ...
            double solveTime = 60 * 60; // Max time to solve the planning problem (seconds)
            
            // ----------- Planner parameters -----------

            int n_iter = 50;  // number of control iterations

            const double Pq = 10.0; // control quaternion error weight
            const double Pw = 0.4; // control angular velocity error weight

            double mu = 2; // weight for torque minimization 
            double lambda = 1; // weight for tension minimization

            double accelMaxDelta = 0.02; // or your chosen maximum delta
            double zScale = 0.4;
...
{
public:
    AccelDeltaControlSampler(const ompl::control::ControlSpace *space, double accelMaxDelta, double zScale)
        : ompl::control::ControlSampler(space), accelMaxDelta_(accelMaxDelta), zScale_(zScale) {}
