...
unsigned int ompl::app::PayloadSystem::droneCount_ = 1; // Number of drones
...
{
    std::cout << "[PayloadSystem] Constructor called." << std::endl;

    // Set the stepper for the ODE solver
    std::cout << "[PayloadSystem] Creating ODE solver..." << std::endl;
    odeSolver = std::make_shared<ompl::control::ODEAdaptiveSolver<StepperType>>(
        ...
        {
            std::cout << "[PayloadSystem] ODE function called." << std::endl;
            ode(q, ctrl, qdot);
        },
    ...
    std::cout << "[PayloadSystem] Setting default bounds..." << std::endl;
    setDefaultBounds();

    std::cout << "[PayloadSystem] Setting propagation step size: " << timeStep_ << std::endl;
    si_->setPropagationStepSize(timeStep_);
    si_->setMinMaxControlDuration(1, 10);

    std::cout << "[PayloadSystem] Setting state propagator..." << std::endl;
    si_->setStatePropagator(control::ODESolver::getStatePropagator(odeSolver,
        ...
        {
            std::cout << "[PayloadSystem] postPropagate called." << std::endl;
    ...
    std::cout << "[PayloadSystem] Setting motion validator..." << std::endl;
    auto motionValidator = std::make_shared<ompl::base::DiscreteMotionValidator>(si_);
    si_->setMotionValidator(motionValidator);

    std::cout << "[PayloadSystem] Calling si_->setup()..." << std::endl;
    si_->setup();

    std::cout << "[PayloadSystem] Constructor finished." << std::endl;
}
...
ompl::base::StateSpacePtr ompl::app::PayloadSystem::constructStateSpace()
{
    ...
    for (unsigned int i = 0; i < droneCount_; ++i)
    {
        ...
        stateSpace->addSubspace(std::make_shared<base::RealVectorStateSpace>(4), 0.2);
    }
...
void ompl::app::PayloadSystem::ode(const control::ODESolver::StateType &q, const control::Control *ctrl, control::ODESolver::StateType &qdot)
{
    ...
    Eigen::Quaterniond payloadRot(q[6], q[3], q[4], q[5]);
    ...
    for (unsigned int i = 0; i < droneCount_; ++i)
    {
        ...
        Eigen::Vector3d corner = payloadRot * Eigen::Vector3d(0, 0, h / 2);
