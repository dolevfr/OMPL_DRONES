...
void payloadSystemDemo(app::PayloadSystem &setup)
{
    ...
    // 1) create and init your Acados MPC interface
    auto acadosSolver = std::make_shared<AcadosMPC>();

    // 2) plug in the MPC-based directed sampler
    si->setDirectedControlSamplerAllocator(
        [&](const ompl::control::SpaceInformation *si)
            -> std::shared_ptr<ompl::control::DirectedControlSampler>
        {
            return std::static_pointer_cast<ompl::control::DirectedControlSampler>(
                std::make_shared<MPCSampler>(si, acadosSolver, /*replanSteps=*/5)
            );
        }
    );
