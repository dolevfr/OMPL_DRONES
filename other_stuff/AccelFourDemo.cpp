...
#include "AccelClasses.h"


using namespace ompl;

void payloadSystemSetup(app::AccelPayloadSystem &setup)
{
    ...
    for (unsigned int i = 0; i < setup.getRobotCount(); ++i)
    {
        ...
        {
            if (j < 2)
            start->as<base::RealVectorStateSpace::StateType>(baseIndex + 2)->values[j] = M_PI_2;
            else
            start->as<base::RealVectorStateSpace::StateType>(baseIndex + 2)->values[j] = 0.0;
        }
    }

    unsigned int accelIdx = stateSpace->as<base::CompoundStateSpace>()->getSubspaceCount() - 1;
    auto *startAccel = start->as<ompl::base::RealVectorStateSpace::StateType>(accelIdx);
    startAccel->values[0] = 0.0;
    startAccel->values[1] = 0.0;
    startAccel->values[2] = 0.0;

    // Print the start state nicely
    std::cout << "====================================" << std::endl;
    ...
    // Payload quaternion
    const auto &payloadQuat = start->as<base::SE3StateSpace::StateType>(0)->rotation();
    std::cout << "Payload quat:  ("
              << std::fixed << std::setprecision(4)
              << payloadQuat.x << ", " << payloadQuat.y << ", "
              << payloadQuat.z << ", " << payloadQuat.w << ")" << std::endl;

    // Payload velocity
    std::cout << "Payload vel:   (";
    for (unsigned int i = 0; i < 3; ++i)
        std::cout << std::fixed << std::setprecision(4) << start->as<base::RealVectorStateSpace::StateType>(1)->values[i]
                  << (i < 2 ? ", " : "");
    std::cout << ")" << std::endl;
    ...
    {
        unsigned int baseIndex = 2 + i * 3;
        const auto *droneQuat = start->as<base::SO3StateSpace::StateType>(baseIndex);
        const auto *droneAngVel = start->as<base::RealVectorStateSpace::StateType>(baseIndex + 1);
        const auto *cableState = start->as<base::RealVectorStateSpace::StateType>(baseIndex + 2);

        std::cout << "Drone " << (i + 1) << " quat:      ("
                  << std::fixed << std::setprecision(4)
                  << droneQuat->x << ", " << droneQuat->y << ", "
                  << droneQuat->z << ", " << droneQuat->w << ")" << std::endl;

        std::cout << "Drone " << (i + 1) << " angvel:    (";
        for (unsigned int j = 0; j < 3; ++j)
            std::cout << std::fixed << std::setprecision(4) << droneAngVel->values[j]
                      << (j < 2 ? ", " : "");
        std::cout << ")" << std::endl;

        std::cout << "Drone " << (i + 1) << " θ, φ:       ("
                  << std::fixed << std::setprecision(4)
                  << cableState->values[0] << ", " << cableState->values[1] << ")" << std::endl;

        std::cout << "Drone " << (i + 1) << " θ̇, φ̇:      ("
                  << std::fixed << std::setprecision(4)
                  << cableState->values[2] << ", " << cableState->values[3] << ")" << std::endl;
    }

    std::cout << "T_f (final time var): "
              << std::fixed << std::setprecision(4)
              << startAccel->values[0] << std::endl;
    ...
    auto *goalAccel = goal->as<ompl::base::RealVectorStateSpace::StateType>(accelIdx);
    goalAccel->values[0] = 0.0;
    goalAccel->values[1] = 0.0;
    goalAccel->values[2] = 0.0;
...
void payloadSystemDemo(app::AccelPayloadSystem &setup)
{
...
int main(int argc, char** argv)
{
    ...
    py::scoped_interpreter guard{};  // Initialize Python interpreter here!

    ompl::app::AccelPayloadSystem sys;
