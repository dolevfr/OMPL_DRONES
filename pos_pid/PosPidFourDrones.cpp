...
void ompl::app::PayloadSystem::postPropagate(const base::State *from, const control::Control *control, double /*duration*/, base::State *to)
{
    ...
    for (unsigned int d = 0; d < droneCount_; ++d) {
        int base = 2 + d*3;
    ...
    // Previous acceleration (3D, last subspace)
    ...
    // Previous payload position (from state)
    ...
    // Sampled absolute target position from control
    ...
    // Direction from current position to the sampled target
    Eigen::Vector3d diff = sampled_target - prev_pos;
    double dist = diff.norm();
    Eigen::Vector3d dir = (dist > 1e-6) ? (diff / dist).eval() : Eigen::Vector3d::Zero();

    Eigen::Vector3d next_accel = accelMaxDelta * dir;
    ...
    // std::cout << "[DEBUG] Initial guess x0: " << Eigen::Map<Eigen::VectorXd>(x0, 12).transpose() << std::endl;

    // // --- Build problem ---
    // ceres::Problem problem;
    // ceres::CostFunction* cost_function =
    //     new ceres::AutoDiffCostFunction<CableResidualCeres, 10, 12>(
    //         new CableResidualCeres(F_req, corners_rel_world, mu, lambda));
...
void ompl::app::PayloadSystem::setDefaultBounds()
{
    ...
    accelBounds.setLow(-1000);
    accelBounds.setHigh(1000);
